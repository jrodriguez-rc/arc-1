...
| SKTD/KTD create (`SAPWrite action=create type=SKTD`) | `src/handlers/write/create.ts` (inline `sktd:docu` body → POST `/sap/bc/adt/documentation/ktd/documents`, CT `sktdv2+xml`), `src/handlers/{schemas,tools}.ts` (`refObjectType`/`refObjectName`/`refObjectDescription`) — `<sktd:refObject adtcore:description>` must NOT be empty: SAP 7.58 answers HTTP 400 "Check of condition failed" for `""` and accepts any non-empty value, so ARC-1 falls back to the KTD `description`, then the name. `refObjectName` must equal the KTD name (one KTD per object). |
